...
| `_CLIENT_VARIABLE` | — | 保存 Mainsail 客户端暂停、取消和停靠相关参数。 | `settings/mainsail_client.cfg` |
...
| `_TOOLCHANGE_DROPOFF` | — | 保存或清除打印位置、复位 offset、回抽、停靠、解锁并将工具切到待机温度。 | `tools/toolgroup.cfg` |
| `_TOOLCHANGE_PICKUP` | — | 解锁耦合器、拾取并锁定工具、等待温度、prime、应用工具 offset/温度补偿并按需恢复位置。 | `tools/toolgroup.cfg` |
...
| `COUPLER_LOCK` | — | 必要时先复位，然后将耦合器移动到锁定位置。 | `tools/coupler.cfg` |
| `COUPLER_UNLOCK` | — | 必要时先复位，然后将耦合器移动到解锁位置。 | `tools/coupler.cfg` |
...
| 命令 | 原始命令 | 功能 | 定义文件 |
| --- | --- | --- | --- |
| `G28` | `G28.0` | 增加热床温度、工具状态、CoreXY 安全位置、电机电流和 Z 避让检查后调用原始归零。 | `macros/G28.cfg` |
...
| `CANCEL_PRINT` | `_CANCEL_PRINT` | 调用被重命名的既有取消流程后卸下工具，并关闭加热器、风扇和电机；既有命令通常由 Mainsail 提供。 | `macros/cancel.cfg` |
